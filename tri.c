...
int main(int argc,char *argv[])
{
	...
	int option,ss,rss,st,tscore,trys,frame,g;
	init_x();
	char dummy[10];
	...
	if (good>0)
	{
		...
		sprintf(jayfile,"./jpegs/rrimage%04d.jpg",frame);
...
int n_score(struct squares *can,struct squares *agn)
{
	...
	total+=testline (can->x3,can->y3,can->x1,can->y1,agn->x1,agn->y1,agn->x2,agn->y2);
	...
	total+=testline (can->x3,can->y3,can->x1,can->y1,agn->x2,agn->y2,agn->x3,agn->y3);
	//total+=testline (can->x4,can->y4,can->x1,can->y1,agn->x2,agn->y2,agn->x3,agn->y3);

	total+=testline (can->x1,can->y1,can->x2,can->y2,agn->x3,agn->y3,agn->x1,agn->y1);
	total+=testline (can->x2,can->y2,can->x3,can->y3,agn->x3,agn->y3,agn->x1,agn->y1);
	total+=testline (can->x3,can->y3,can->x1,can->y1,agn->x3,agn->y3,agn->x1,agn->y1);
	...
	total=(total*2000)/(1+(dx*dx)+(dy*dy));
...
{
	int w,h,angle,x1,x2,x3,y1,y2,y3,xp,yp;
	float l,theta;
	...
	yp=sq->yp;
        ...
        if (!(x1<X_SIZE && x2<X_SIZE && x3<X_SIZE && x1>=0 && x2 >=0 && x3 >=0 && 
        y1<Y_SIZE && y2<Y_SIZE && y3<Y_SIZE && y1>=0 && y2 >=0 && y3 >=0 ))
        {
		return 0;
        }
	sq->x1=x1;
...
void allocate(struct squares *sq)
{
		...
		cposs=0;
			...
			if ((((r-g)*(r-g))+((b-r)*(b-r))+((g-b)*(g-b)))>30000 ){cposs=1;}
...
void place(struct squares *sq)
{
	...
	line(sq->x3,sq->y3,sq->x1,sq->y1,sq->r,sq->g,sq->b);
